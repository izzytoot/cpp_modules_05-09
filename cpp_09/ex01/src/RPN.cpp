...
int RPN::getResult() {
    if (this->_stack.size() != 1)
        throw std::runtime_error(RED"Error: more than one element in stack."RES);
...
void RPN::performOperation() {
    ...
    for (size_t i = 0; i < this->_input.size(); i++){
        ...
        if (this->_input[i] == '-' && (this->_input[i + 1] && (this->_input[i + 1] >= '0' && this->_input[i] <= '9')))
            throw std::runtime_error(RED"Error: bad input => number < 0."RES);
            
        if (this->_input[i] == '+' || this->_input[i] == '-' || this->_input[i] == '*' || this->_input[i] == '/'){
            int result = doTheMath(this->_input[i]);
            this->_stack.push(result);
            continue;
        }

        if (this->_input[i] != ' ' && (this->_input[i] < '0' && this->_input[i] > '9'))
